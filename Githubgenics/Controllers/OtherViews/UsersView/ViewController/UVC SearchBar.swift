...
extension UsersViewController  : UISearchBarDelegate  {
    ...
    func searchBarTextDidBeginEditing(_ searchBar: UISearchBar) {
        ...
        DispatchQueue.main.async {
            ...
            if self.searchController.searchBar.text?.isEmpty == true , self.viewModel.lastSearch.isEmpty == true {
                self.conditionLabel.isHidden = false
            } else {
                self.conditionLabel.isHidden = true
            }
            self.loadingSpinner.dismiss()
            self.viewModel.recentSearchData(collectionView: self.collectionView, tableView: self.tableView)
        }
    }
