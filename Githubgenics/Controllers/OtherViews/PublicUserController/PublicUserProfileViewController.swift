...
class PublicUserProfileViewController: ProfileSetup  {
...
extension PublicUserProfileViewController {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.row == 0 {
            let vc = UIStoryboard.init(name: Storyboards.reposView , bundle: Bundle.main).instantiateViewController(withIdentifier: ID.usersRepositoryViewControllerID) as? PublicUserRepositoriesViewController
            vc?.passedUser = passedUser
            self.navigationController?.pushViewController(vc!, animated: true)
            
        } else if indexPath.row == 1 {
            let vc = UIStoryboard.init(name: Storyboards.starredView , bundle: Bundle.main).instantiateViewController(withIdentifier: ID.publicUserStarredViewControllerID) as? PublicUserStarredViewController
            vc?.passedUser = passedUser
            self.navigationController?.pushViewController(vc!, animated: true)
