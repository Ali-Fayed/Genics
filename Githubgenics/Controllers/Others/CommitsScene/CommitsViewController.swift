...
class CommitsViewController: UIViewController {
    ...
    func renderCommits() {
      guard let repository = repository else {
        ...
        GitAPIcaller.shared.makeRequest(returnType: [Commit].self, requestData: GitRequsetRouter.gitRepositoriesCommits(repository.repoFullName), pagination: false) { [weak self] (commits) in
    ...
    func renderCachedReposCommits() {
      ...
      guard let repository = savedRepos else {
        ...
        GitAPIcaller.shared.makeRequest(returnType: [Commit].self, requestData: GitRequsetRouter.gitRepositoriesCommits(repository.repoUserFullName ?? ""), pagination: false) { [weak self] (commits) in
            self?.commits = commits
            self?.spinner.dismiss()
            self?.tableView.reloadData()
          }
