...
struct GitSecureStore {
  ...
  func setValue(_ value: String, for userAccount: String) throws {
    guard let encodedPassword = value.data(using: .utf8) else {
      throw GitSecureStoreError.stringToDataConversionError
    }
    ...
    case errSecSuccess:
      guard
        ...
        let password = String(data: passwordData, encoding: .utf8)
          ...
          throw GitSecureStoreError.dataToStringConversionError
      }
      return password
    case errSecItemNotFound:
      return nil
  ...
  func error(from status: OSStatus) -> GitSecureStoreError {
    let message = SecCopyErrorMessageString(status, nil) as String? ?? NSLocalizedString("Unhandled Error", comment: "")
    return GitSecureStoreError.unhandledError(message: message)
  }
}
