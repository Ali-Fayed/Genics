...
extension BookmarksViewController: UITableViewDelegate , UITableViewDataSource {
    ...
    func openUserInSafari (indexPath: IndexPath) {
        guard let url = self.viewModel.getUsersViewModel(at: indexPath).userURL else {return}
        let vc = SFSafariViewController(url: URL(string: url)!)
    ...
    func openRepoInSafari (indexPath: IndexPath) {
        guard let url = self.viewModel.getReposViewModel(at: indexPath).repoURL else {return}
        let vc = SFSafariViewController(url: URL(string: url)!)
        self.present(vc, animated: true)
    }
