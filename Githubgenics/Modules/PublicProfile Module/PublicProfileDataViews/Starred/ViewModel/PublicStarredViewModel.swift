...
class PublicStarredViewModel {
    ...
    func loadStarred (tableView: UITableView, view: UIView, loadingSpinner: JGProgressHUD, conditionLabel: UILabel ) {
        ...
        GitAPIcaller.makeRequest(dataModel: [Repository].self, requestData: GitRequestRouter.gitPublicUsersStarred(pageNo, repository.userName)) { [weak self] (result) in
    ...
    func fetchMoreStarredRepos (at indexPath: IndexPath, tableView: UITableView, tableFooterView: UIView, loadingSpinner: JGProgressHUD) {
        ...
        if indexPath.row == numberOfStarredCells - 1 {
            if pageNo < totalPages {
                ...
                GitAPIcaller.makeRequest(dataModel: [Repository].self, requestData: GitRequestRouter.gitPublicUsersStarred(pageNo, user.userName), pagination: true) { [weak self]  (moreStarredRepos) in
                    DispatchQueue.main.async {
